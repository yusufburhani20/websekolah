<div x-data="{ open: false }" @open-modal.window="if ($event.detail === 'applyModal') open = true" @close-modal.window="if ($event.detail === 'applyModal') open = false">
    
    <!-- Trigger Modal Button -->
    <button type="button" @click="open = true" class="w-full bg-white text-brand-600 hover:bg-brand-50 font-bold py-3.5 px-6 rounded-xl transition-all shadow-md group flex items-center justify-center">
            ...
            <div x-show="open" 
                ...
                @if($successMessage)
                    <div class="p-8 text-center">
                        ...
                        <h4 class="font-bold text-2xl text-slate-800 mb-2">Lamaran Terkirim!</h4>
                        <p class="text-slate-600 mb-6">Terima kasih, lamaran dan CV Anda telah berhasil dikirimkan. Semoga beruntung!</p>
                        <button type="button" @click="open = false" class="px-8 py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl transition-all shadow-md">Tutup Formulir</button>
                    </div>
                @else
